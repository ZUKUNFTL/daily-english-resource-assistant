...
class PyVideoTransTranslator(LocalTranslator):
    ...
    def translate(self, texts: list[str]) -> list[str]:
        if not texts:
            return []
        temp_root = self.runner.root / "work" / "tmp"
...
class ArgosTranslator(LocalTranslator):
    def __init__(self, root: str | Path, source_language: str, target_language: str) -> None:
        self.root = Path(root)
        self.source_language = source_language
        self.target_language = target_language
    ...
    def available(self) -> bool:
        package_root = self.root / "work" / "models" / "argos"
        if not package_root.exists():
            return False
        return self.python_path.exists() and self.script_path.exists() and any(package_root.iterdir())
    ...
    def translate(self, texts: list[str]) -> list[str]:
        ...
        environment["XDG_DATA_HOME"] = str(self.root / "work" / "data")
        environment["XDG_CONFIG_HOME"] = str(self.root / "work" / "config")
        environment["XDG_CACHE_HOME"] = str(self.root / "work" / "cache")
        environment["ARGOS_PACKAGES_DIR"] = str(self.root / "work" / "models" / "argos")
        ...
        result = subprocess.run(
            [str(self.python_path), str(self.script_path)], input=request, capture_output=True,
...
class CaptionPipeline:
    ...
    def transcribe(
        ...
        if self.runner.status().available:
        ...
        cache_root = self.runner.root / "work" / "cache"
        temp_root = self.runner.root / "work" / "tmp"
        ...
        if progress:
            progress(-1.0, f"正在加载或下载 {model_name} 模型，请稍候…")
        ...
        project.subtitle_source = f"本地 Whisper {model_name} 转写"
        project.status = ProjectStatus.REVIEW
        return project
    ...
    def translate_missing(
        ...
        if translator is None:
            argos = ArgosTranslator(self.runner.root, source_language, target_language)
            if argos.available():
                translator = argos
    ...
    def export_package(self, project: Project, destination: str | Path, create_zip: bool = False) -> Path:
